...
namespace ChessAI
{
    class NegaMaxMasterThread
    {
        ...
        public NegaMaxMasterThread(Board board, bool color)
        {
            this.board = board;
            this.color = color;
        ...
        public Move Run()
        {
            ...
            {
                NegaMaxThread thread = new NegaMaxThread(board.Clone(), color, this);
            ...
            if (alpha > this.alpha)
            {
                lock (_lockerStore)
                {
                    if (alpha > this.alpha)
                    {
                        moveToMake = move;
                        this.alpha = alpha;
                        Console.WriteLine("New move:" + alpha + " @depth:" + 6);
                    }
                }
            }
        }
